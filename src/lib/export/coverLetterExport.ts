/**
 * Client-side cover letter export — replaces /api/export-cover-letter.
 * Generation logic (pdf-lib, docx, txt) is unchanged; the file is built
 * and saved entirely in the browser. No auth or usage limits — the app
 * is free.
 */
...
/** Generates the cover letter file and triggers a download. Throws on failure. */
export async function exportCoverLetter(data: CoverLetterData): Promise<void> {
  const {
    content,
    format,
    applicantName,
  ...
  if (!content) {
    throw new Error('Missing cover letter content')
  }
  ...
  if (format === 'pdf') {
    ...
    saveAs(blob, `Cover-Letter-${safeCompanyName}.pdf`)
  } else if (format === 'docx') {
    ...
    saveAs(blob, `Cover-Letter-${safeCompanyName}.docx`)
  } else {
    ...
    saveAs(blob, `Cover-Letter-${safeCompanyName}.txt`)
  }
}
