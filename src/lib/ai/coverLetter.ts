/**
 * Cover letter service — browser port of the old /api/generate-cover-letter
 * and /api/refine-cover-letter routes.
 *
 * Prompts, post-processing and validation are preserved verbatim from the
 * server routes. Auth, usage limits, tier gating, and capture pipelines are
 * gone — the user's own Anthropic key (see Settings) does the work.
 */
...
/**
 * Generate a cover letter. Mirrors the old POST /api/generate-cover-letter.
 */
export async function generateCoverLetter(input: CoverLetterInput): Promise<CoverLetterResult> {
  ...
  if (!jobData?.company || !jobData?.title || !jobData?.description) {
    throw new Error('Missing job details: company, title, and description are required')
  }
  ...
  if (selectedAchievements && selectedAchievements.length > 0) {
    ...
    achievements = experiences
      ?.flatMap((exp) =>
        exp.bullets
          ?.filter((b) => b.translated_text || b.original_text)
          .map((b) => (b.translated_text || b.original_text) as string) || []
      )
      .slice(0, 5) || []
  }
...
/**
 * Post-process refined cover letter to remove any AI patterns that slipped
 * through. (The refine route shipped its own variant — kept verbatim.)
 */
function cleanRefinedCoverLetter(text: string): string {
  let cleaned = text
...
/**
 * Validate refined cover letter quality and flag issues.
 * (The refine route shipped its own variant — kept verbatim.)
 */
function validateRefinedCoverLetter(text: string): { valid: boolean; issues: string[] } {
  const issues: string[] = []
...
export interface RefineCoverLetterInput {
  action: string
...
/**
 * Refine an existing cover letter. Mirrors the old POST /api/refine-cover-letter.
 */
export async function refineCoverLetter(input: RefineCoverLetterInput): Promise<RefineCoverLetterResult> {
  const anthropic = getAnthropicClient()

  const { action, currentLetter } = input
...
Output the version with more numbers:`,
  ...
  refined = cleanRefinedCoverLetter(refined)

  // Validate the result
  const validation = validateRefinedCoverLetter(refined)
