/**
 * Job match analysis — browser port of the old /api/job-match and
 * /api/job-match/suggestions routes.
 *
 * Preserves the full scoring prompt, enforceScoring() post-processing,
 * JSON repair fallback, the 24h in-memory cache, and request timeouts.
 * Profile context now comes from local storage; occupation lookup uses the
 * bundled local crosswalk instead of the live O*NET API.
 */
import type Anthropic from '@anthropic-ai/sdk'
import { searchOccupations } from '@/lib/ai/occupations'
import { getCivilianJobs } from '@/lib/debriefed-token-saver/jobCrosswalk'
import { dictionaryTranslate } from '@/lib/translation-engine'
import { getAnthropicClient, trackTokens, PRIMARY_MODEL } from '@/lib/ai/client'
import { getProfile } from '@/lib/storage'

// Create cache key from resume + job description
function createCacheKey(resumeContent: any, jobDescription: string): string {
  return `job_match:${JSON.stringify(resumeContent)}::${jobDescription}`
}
...
// Find the closest occupation for a job title via the local crosswalk
// (the old O*NET skills lookup was a no-op and has been removed)
...
export async function analyzeJobMatch(input: JobMatchInput): Promise<JobMatchResult> {
  ...
  // Get profile data for prompt context
  ...
  if (cachedEntry && cachedEntry.expires > Date.now()) {
    ...
    return { analysis: cachedEntry.result, cached: true }
  }
  ...
  // Fetch occupation context for job title (async, with timeout)
  let onetContext = ''
  try {
    ...
    // Ignore occupation lookup failures
  }
...
`
  ...
  try {
    ...
    if (err.name === 'AbortError' || err.message?.includes('abort')) {
      throw new Error('Analysis timed out. Try a shorter job description or resume.')
    }
    throw err
  } finally {
  ...
  // Ensure empty defaults for fields handled by getJobMatchSuggestions
  ...
  // Track token usage (courtesy local meter)
...
function enforceScoring(analysis: any): any {
  ...
  const baseScore = totalWeight > 0 ? Math.round(weightedScore / totalWeight) : 50
...
// ============================================================================
// Suggestions — port of /api/job-match/suggestions
// ============================================================================

export interface JobMatchSuggestionsInput {
  resumeContent: any
  jobPosting: JobPostingInput
  gaps?: any[]
}

export interface JobMatchSuggestionsResult {
  bulletSuggestions: any[]
  skillChanges: any
}

export async function getJobMatchSuggestions(
  input: JobMatchSuggestionsInput
): Promise<JobMatchSuggestionsResult> {
  const { resumeContent, jobPosting, gaps } = input
...
Return ONLY valid JSON:
{
  ...
  try {
    ...
    if (err.name === 'AbortError' || err.message?.includes('abort')) {
      throw new Error('Suggestions request timed out')
    }
    throw err
  } finally {
    clearTimeout(timeout)
