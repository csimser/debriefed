...
export async function GET(request: Request) {
  ...
  // For recovery flow, check if we have a valid session (either from code exchange or cookie-based)
  if (type === 'recovery') {
    const { data: { session } } = await supabase.auth.getSession()
    if (session) {
      return NextResponse.redirect(`${origin}/reset-password`)
    }
    // No session for recovery - something went wrong
    console.error('Recovery flow: No session found')
    return NextResponse.redirect(`${origin}/login?error=recovery_failed`)
  }

  // For non-recovery flows, we need a code to have been exchanged
  if (!code) {
    return NextResponse.redirect(`${origin}/login?error=auth_failed`)
  }
